...
def find_contact_email(name: str) -> str:
    """Find email address for a contact by name from the contacts.csv file.
    
    Args:
        name (str): The name of the contact to search for. Can be partial name.
    
    Returns:
        str: The email address if found, or a message indicating no match was found.
    
    Raises:
        FileNotFoundError: If contacts.csv file doesn't exist.
    """
    ...
    try:
        with open(contacts_file, 'r', newline='') as file:
            ...
            # Search for exact or partial name match (case-insensitive)
            for row in all_rows:
                if name.lower() in row['Name'].lower():
                    return f"Found contact: {row['Name']} - Email: {row['Email']}"
            
            # If no match found, list available contacts
            available_names = [row['Name'] for row in all_rows]
            return f"No contact found with name containing '{name}'. Available contacts: " + \
                   ", ".join(available_names)
                   
    except Exception as e:
        return f"Error reading contacts file: {str(e)}"

@function_tool
def list_all_contacts() -> str:
    """List all contacts from the contacts.csv file.
    
    Returns:
        str: A formatted list of all contacts with their names and emails.
    ...
    try:
        with open(contacts_file, 'r', newline='') as file:
            ...
            contacts = []
            for row in reader:
                contacts.append(f"- {row['Name']}: {row['Email']}")
            
            if contacts:
                return "Available contacts:\n" + "\n".join(contacts)
            else:
                return "No contacts found in the contacts file."
